...
import pandas as pd
import numpy as np
import soundfile as sf  # <--- IMPORT ADDED
...
# component 1: The Dataset Loader
class ESC50Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load Audio File
        wav_numpy, sample_rate = sf.read(str(audio_path))
...
#component 2: Mixup Augmentation
def mixup_data(x, y):
    lam = np.random.beta(0.2, 0.2)
    batch_size = x.size(0)
    index = torch.randperm(batch_size).to(x.device)
    # Formula: New_Image = (λ * Image_A) + ((1-λ) * Image_B)
...
def train():
    ...
    # Define the Audio-to-Image Pipeline
